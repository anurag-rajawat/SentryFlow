...
func TestManager_eventsHandler(t *testing.T) {
	...
	validApiEvent := getValidApiEvent()
	invalidApiEvent := getInvalidApiEvent()
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			// Avoid using `t.Cleanup()` as it's executed after all subtests complete. To
